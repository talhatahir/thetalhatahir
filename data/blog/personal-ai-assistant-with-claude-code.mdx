...
Open [Obsidian](https://obsidian.md/) → Open folder as vault → select the `claude-assistant` folder. Now you can see all your memory files in a beautiful interface. When your assistant writes a new memory, pull and it's right there.
...
```bash
echo "alias assistant='cd /full/path/to/claude-assistant && claude'" >> ~/.zshrc
source ~/.zshrc
```

Now your Mac reads the same identity files, connects to the same Google Calendar, writes to the same GitHub memory. **Same assistant, different machine.**
