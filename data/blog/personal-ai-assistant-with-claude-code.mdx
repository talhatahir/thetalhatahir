...
images:
  ['https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&w=1161&q=80']
---

![AI Assistant](https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&w=1161&q=80)

## TL;DR 🎯

I replaced my [OpenClaw setup](../blog/my-simple-openclaw-installation-journey) with a Claude Code session running 24/7 on a €5/month Hetzner VPS. Discord as the chat interface (works on iPhone), Google Calendar for daily briefings, GitHub repo as the memory layer (viewable in Obsidian). No Docker containers, no token burn surprises, no stale bots. **Total cost: ~$26/month.** One focused afternoon to set up.
...
- A GitHub repo as the memory layer, viewable in Obsidian
...
Clone the same repo on your Mac:

```bash
git clone https://github.com/yourusername/claude-assistant.git
```

Open [Obsidian](https://obsidian.md/) → Open folder as vault → select the `claude-assistant` folder. Now you can see all your memory files in a beautiful interface. When Claudia writes a new memory, pull and it's right there.
...
```bash
echo "alias claudia='cd ~/claude-assistant && claude --channels plugin:discord@claude-plugins-official'" >> ~/.bashrc
source ~/.bashrc
```
...
Add an alias (no `--channels` flag — Discord runs from the VPS only):

```bash
echo "alias claudia='cd /full/path/to/claude-assistant && claude'" >> ~/.zshrc
source ~/.zshrc
```

Now type `claudia` on your Mac and she reads the same identity files, connects to the same Google Calendar, writes to the same GitHub memory. **Same assistant, different machine.**
...
**Termius on iPhone** — for when you need terminal access:
1. Install [Termius](https://termius.com/) from App Store
2. Tailscale on iPhone, same account
3. Add your VPS Tailscale IP as a host
4. Connect and run `tmux attach -t assistant`

I rarely need Termius. Discord handles 95% of interactions.
...
Every write is git committed and pushed. Your Mac pulls it. Obsidian shows it. GitHub stores it. Everything in sync, no iCloud, no Notion, no extra services.
...
One DM thread gets messy fast. Create channels in your server for different purposes:

- **#general** — casual conversation
- **#briefings** — daily calendar summaries
- **#blog** — blog writing requests and drafts
- **#fitness** — gym check-ins

Get each channel ID (right-click → Copy Channel ID with Developer Mode on) and add them to CLAUDE.md:

```markdown
## Discord channels
- DM: general conversation
- #blog (ID: your-id-here): all blog writing
- #briefings (ID: your-id-here): daily calendar summaries
```
...
GitHub repo (claude-assistant)
  └── synced to Mac + viewed in Obsidian
...
- **Fitness tracking** — the 11pm cron asks if you worked out and logs it to `memory/fitness-log.md`
- **Blog writing** — connect the GitHub MCP, point it at your blog repo, ask it to write and commit drafts
